...
// This is a modified version of the StampMaker class from the original project https://github.com/kozo002/px-brush

let createCanvas: any;
if (typeof window === "undefined") {
  import("@napi-rs/canvas").then((module) => {
...
export default class StampMaker {
  canvases: Record<string, Record<number, HTMLCanvasElement>> = {};
  constructor() {
    this.canvases = {};
  }
  ...
  parseColor(color: string) {
    ...
    if (!isHex && !isRGB) {
      throw new Error(
        "Color is not correct format. #123123 or rgb(123, 123, 123) format required."
      );
    }
    if (isHex) {
      ...
      c =
        c.length === 3
          ? c
              .split("")
              .reduce((acc: string[], it: string) => [...acc, it, it], [])
              .join("")
          : c;
  ...
  make({ size, color }: { size: number; color: string }) {
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        canvas = createCanvas(size, size) as unknown as HTMLCanvasElement;
      }
  ...
  fillCircle(imageData: ImageData, color: { r: number; g: number; b: number }) {
